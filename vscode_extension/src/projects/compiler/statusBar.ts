...
export class CompilerPicker {
  ...
  constructor() {
    ...
    if (workspace.workspaceFolders && workspace.workspaceFolders.length > 0) this.statusBarItem.show();
  ...
  public async updateDisplay(): Promise<void> {
    try {
      ...
      const currentConfigName = config.groupProjectsCompiler || 'No Compiler';
